...
impl<'a> Lexer<'a> {
	...
	pub fn determine_indent(&mut self) {
		...
		if self.line_info.last_line != self.line_info.current_line {
			match spaces > self.line_info.last_spaces {
				true => {
					self.create_token(ToT::Indent);
					self.line_info.last_spaces = spaces;
				}
				false => {
					if let Some(atom) = self.latest() {
						match atom {
							Atom::Token(token) => {
	...
	#[must_use = "If you would just like to lex, without getting the \
	              latest token, just run `.lex()`."]
